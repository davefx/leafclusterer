...
L.Marker.Clusterer = L.Class.extend({
  ...
  initialize: function (map, opt_markers, opt_opts) {
    ...
    this._map._panes.clusterPane = this._map._createPane("leaflet-cluster-pane");
    this._clusterPane = document.getElementsByClassName("leaflet-cluster-pane")[0];
    this._clusterPane.style.zIndex = 8;
    this._clusterPane.style.position = "absolute";
  ...
  */
  removeMarker: function (marker) {
    for (var i = 0; i < this._leftMarkers.length; ++i) {
      if (marker === this._leftMarkers[i]) {
        this._leftMarkers.splice(i, 1);
        return;
      }
    }
    for (var i = 0; i < this._clusters.length; ++i) {
      if (this._clusters[i] && this._clusters[i].removeMarker(marker)) {
        if (clusters_[i].getTotalMarkers() == 0) {
          this._clusters[i].clearMarkers();
          this._clusters.splice(i, 1);
        } else {
          this._clusters[i].redraw_();
        }
        return;
      }
    }
...
L.Marker.ClusterMarker = L.Class.extend({
  initialize: function(cluster, styles) {
    ...
    this.reset({latLng:cluster.getCenter(),count: cluster.getTotalMarkers(),styles: styles,padding: cluster._clusterer.getGridSize() / 2});
  },
  ...
  redraw: function() {
    ...
    //this is important!
    this._container.style.zIndex = pos.y;
    this._container.style.position = "absolute";
    
    if (L.Browser.webkit3d) {
      this._container.style[L.DomUtil.TRANSFORM] = L.DomUtil.getTranslateString(pos);
    } else {
